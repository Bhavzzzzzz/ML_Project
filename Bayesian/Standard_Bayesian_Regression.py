...
# =============================================================================
# STEP 2 & 3: DEFINE AND TRAIN THE PYMC MODEL (MODIFIED)
# =============================================================================
...
summary = az.summary(idata, var_names=['b0_intercept', 'b_features', 'sigma'])
summary.index = ['Intercept'] + feature_names + ['Model Error (sigma)']
print("\nModel Parameter Summary:")
print(summary)
print("\nHow to read this table:")
print(" - 'mean': The most likely value for the parameter's effect.")
print(" - 'hdi_3%' & 'hdi_97%': The 94% Highest Density Interval (HDI).")
print(" - If the HDI range for a feature *does not* include 0, it's a significant predictor.")

# =============================================================================
# STEP 5: PREDICT ON TEST DATA (CORRECTED)
# =============================================================================
print("\n--- Step 5: Generating Predictions on Test Data ---")

# 1. Extract the posterior samples from our results
posterior = idata.posterior
b0_samples = posterior['b0_intercept'].values.flatten()
b_samples = posterior['b_features'].values.reshape(-1, n_features) # (n_samples, n_features)
sigma_samples = posterior['sigma'].values.flatten()

# Get the total number of posterior samples (e.g., 2000)
n_samples = len(b0_samples)

# 2. Manually calculate 'mu' for the *test set*
mu_pred = b0_samples[:, None] + b_samples @ X_test_scaled.T

# 3. Generate new 'y' values from the likelihood
posterior_predictive_dist = pm.Normal.dist(mu=mu_pred, sigma=sigma_samples[:, None])

# We draw ONE sample from each of the distributions
# The result already has the correct shape: (n_samples, n_test_people)
y_posterior_pred = pm.draw(posterior_predictive_dist, draws=1)

# We no longer need to .squeeze()
# y_posterior_pred = y_posterior_draws.squeeze(0) # <--- REMOVED
# --- END OF FIX ---

print("Prediction generation complete.")

# --- Analyze Predictions ---
# y_posterior_pred now has shape (n_samples, n_test_points)
test_index = 0

# Get all posterior predictions for this one person
all_preds_for_one = y_posterior_pred[:, test_index]

# Get the mean, and the HDI
mean_pred = all_preds_for_one.mean()
pred_hdi = az.hdi(all_preds_for_one, hdi_prob=0.94)

actual_value = y_test.iloc[test_index]

print("\n--- Example Prediction (First Person in Test Set) ---")
...
# Make sure you have downloaded 'test.csv' from the Kaggle competition page
try:
    test_df = pd.read_csv('test.csv')
    print("Successfully loaded 'test.csv'.")
except FileNotFoundError:
    print("\n*** ERROR: 'test.csv' not found. ***")
    print("Please download 'test.csv' from the Kaggle competition page and place it in the same folder.")
    exit()

# --- 2. Store the 'Id' column for the submission file ---
test_ids = test_df['Id']

# --- 3. Preprocess the test_df EXACTLY like the training data ---
print("Preprocessing official test data...")
# Map 'Lifestyle Activities'
test_df['Lifestyle Activities'] = test_df['Lifestyle Activities'].map({'Yes': 1, 'No': 0})

# Ensure columns are in the same order as when you trained
test_df_processed = test_df[feature_names]

# Use the *same* scaler from Step 1 (it's already fitted)
...
# Make sure to round predictions if the competition requires integers
