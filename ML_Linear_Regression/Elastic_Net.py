...
# ============================
# 4️⃣ Handle missing values (only numerics, exclude 'Id' and target)
# ============================
numeric_cols = train_df.select_dtypes(include=[np.number]).columns.drop(['Id', 'Recovery Index'], errors='ignore').tolist()
train_df[numeric_cols] = train_df[numeric_cols].fillna(train_df[numeric_cols].mean())
test_df[numeric_cols] = test_df[numeric_cols].fillna(train_df[numeric_cols].mean())  # Use train means to avoid leakage

# ============================
# 5️⃣ Split data (exclude 'Id')
# ============================
...
# ============================
# 6️⃣ ElasticNet + Hyperparameter Tuning (no scaling)
# ============================
enet = ElasticNet(max_iter=5000)  # Higher iter for convergence

# Grid for alpha (total reg) and l1_ratio (0=Ridge, 1=Lasso)
param_grid = {
...
enet_best = ElasticNet(alpha=best_params['alpha'], l1_ratio=best_params['l1_ratio'], max_iter=5000)
enet_best.fit(X_train, y_train)

# ============================
# 7️⃣ Evaluate
# ============================
y_pred = enet_best.predict(X_val)
...
# ============================
# 8️⃣ Predict on test.csv (exclude 'Id' for prediction)
# ============================
test_features = test_df[feature_cols]
test_pred = enet_best.predict(test_features)
test_pred = np.clip(test_pred, 10, 100)  # Unrounded floats, clipped to [10,100]

# ============================
# 9️⃣ Create submission.csv (use test 'Id', ignore sample length)
# ============================
submission = pd.DataFrame({
    "Id": test_df["Id"],  # Correct column name: 'Id' (not 'ID')
    "Recovery Index": test_pred
})
submission.to_csv("submission_enet.csv", index=False)

print("✅ submission_enet.csv created successfully!")
